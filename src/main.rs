...
fn config_dir() -> PathBuf {
    ...
    PathBuf::from(home).join(".claude-forge")
}
...
fn ensure_claude_desktop_gateway() {
    ...
    if target_id == our_id {
        ...
        if !already_exists {
            ...
            new_entries.push(serde_json::json!({"id": our_id, "name": "ClaudeForge"}));
...
fn autostart_plist_path() -> PathBuf {
    ...
    PathBuf::from(home).join("Library/LaunchAgents/com.claudeforge.plist")
}
...
fn start_server() -> Result<(), String> {
    ...
    rt.block_on(async {
        ...
        eprintln!("ClaudeForge v1.0");
        eprintln!("Local Claude Desktop Model Proxy");
...
fn main() {
    ...
    if let Some(err) = server_err.lock().unwrap().as_ref() {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let _ = std::process::Command::new("osascript")
                .args(["-e", &format!("display dialog \"{}\" buttons {{\"OK\"}} with title \"ClaudeForge\" with icon stop", safe_err)])
                .output();
        }
        std::process::exit(1);
    }
